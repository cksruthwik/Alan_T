...
> **Implementation gate (ADR-016):** before building this custom, a time-boxed Mem0 spike is run with this document as the requirements checklist (taxonomy, supersede chains, confidence gating, review/forget/export). Adopt if it clears without fighting it; build custom otherwise. Either way, this document is the contract the Memory agent exposes.

## 1. The Three Stores

| Layer | Store | Lifetime | Contents |
|---|---|---|---|
| **Short-term** | Redis | session / hours | conversation window, active task context, scratch state |
| **Long-term** | Postgres | permanent (user-controlled) | facts, preferences, goals, projects, lessons, episodic summaries |
| **Semantic** | Qdrant | permanent (re-derivable) | embeddings of documents, code, conversation summaries, memory items |

All behind ports (`CacheStore`, `RelationalStore`, `VectorStore`) — see [ADD.md](../architecture/ADD.md) §3. Postgres is the source of truth for memory; Qdrant holds derived vectors for similarity recall and can always be rebuilt from Postgres + sources.
...
- `lesson` — operational learning from Reflection ([REFLECTION_ENGINE.md](../ai/REFLECTION_ENGINE.md) §4)

**Source ∈** `explicit` (user said "remember…"), `extracted` (post-turn extraction), `consolidated` (nightly job), `reflection`.
...
3. **Consolidation:** nightly job — [MEMORY_CONSOLIDATION.md](MEMORY_CONSOLIDATION.md).
...
At `load_context` ([WORKFLOWS.md](../architecture/WORKFLOWS.md) W1):
...
- Review surface (UI, Phase 2+): flagged low-confidence extractions await confirm/reject.
...
- Not a scratchpad — task state lives in Redis/LangGraph checkpoints and dies with the task.

Schema details: [POSTGRES_SCHEMA.md](../data/POSTGRES_SCHEMA.md). Vector side: [QDRANT_SCHEMA.md](../data/QDRANT_SCHEMA.md).
