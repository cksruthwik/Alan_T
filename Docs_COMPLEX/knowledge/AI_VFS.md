...
- Mirror sync: content hash comparison means unchanged files cost nothing (idempotent `put`); deletions become tombstones; **the user's notes gain version history as a side effect** ("what did this note say last month?" becomes answerable — rollback/versions exposed as a Phase 3+ tool).
...
1. **Ingest ledger keys on BLAKE3** — the same hash ai-vfs already computes per version; `stat` is one metadata read, no re-hashing ([INGESTION_PIPELINE.md](INGESTION_PIPELINE.md) §3).
...
- **A second schema to migrate:** ai-vfs owns its Alembic migrations; Alan_T pins the library version and upgrades deliberately ([DEPLOYMENT.md](../infra/DEPLOYMENT.md) update procedure applies).

## 6. Contract Rules (revised)

1. Agents never import `ai_vfs` directly — everything flows through `FileSource`/`WorkspaceStore` ports ([ADD.md](../architecture/ADD.md) layering rules). The ports keep ai-vfs swappable in principle, even though it's a sibling project we control.
