...
1. Personal corpus (notes, code, PDFs, emails) and memory store — the crown jewels.
2. Credentials: provider API keys, calendar OAuth tokens, Telegram bot token.
3. Action capability: browser, desktop, calendar — an attacker who steers Alan_T steers the user's machine.
...
3. Log pipeline applies redaction filters (key patterns, bearer tokens) before write ([LOGGING.md](../observability/LOGGING.md) §4).
...
5. VFS exclusions (`.env*`, `*.key`, `secrets/`) are the first line against secrets entering the corpus → cloud ([AI_VFS.md](../knowledge/AI_VFS.md) §3).

## 5. Cloud Data Exposure (the ADR-001 trade, managed honestly)

What leaves the machine: chat turns, retrieved chunks in context, chunk text at embedding time, audio for STT/TTS/live, images for vision. Free tiers may use submitted data for training — assume they do.
...
2. **Minimum tools per dispatch:** supervisor strips the tool set to what the routed task needs; a poisoned PDF being summarized meets an agent that has no browser or calendar tools to abuse.
...
4. **Egress guard for browser agent:** form-fill/submission to domains not in the task's origin set requires explicit approval (exfiltration-via-form containment, [BROWSER_AGENT.md](../agents/BROWSER_AGENT.md)).
