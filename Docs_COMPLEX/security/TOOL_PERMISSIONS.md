...
1. Preview shows *exactly* what will happen, including target (domain, file path, recipient).
2. Batch approvals allowed at plan level ([PLANNING_ENGINE.md](../ai/PLANNING_ENGINE.md) §4) — but any step whose args materially changed since plan approval re-asks.
3. Timeout on pending approvals (default 1h) → step fails as `permission_denied`, task replans or aborts; never executes stale approvals.
...
| Phase | Posture |
|---|---|
| 1–3 (read-heavy) | almost everything ALLOW except `ingest_path`, `forget_memory` |
| 4 (vision) | captures ASK (cloud egress of screen/camera — [DECISION_LOG.md](../architecture/DECISION_LOG.md) ADR-006) |
| 5 (browser) | reads ASK→promotable per domain; interactions always ASK initially |
| 6 (desktop) | everything ASK + dry-run mode default-on for the first weeks |
| 7 (autonomy) | plan-level batch approval; session overrides become the main flow |
| 8 (remote) | approvals via Telegram inline buttons; same rules, different surface |
...
- Skill-bundled `scripts/` execute only in the sandboxed executor (ai-vfs Monty — VFS callbacks only, budget-capped), never host shell. This keeps skills clear of the DENY'd `shell_exec` line.
