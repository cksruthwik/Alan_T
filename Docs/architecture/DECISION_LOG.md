...
**Decision:** LangGraph with Postgres checkpointer. Kept behind an orchestration adapter — node functions contain no business logic ([ORCHESTRATION.md](ORCHESTRATION.md) §1).
...
4. **Composes, never escalates:** a skill *orchestrates* tools but every tool call still passes the permission gate ([TOOL_PERMISSIONS.md](../security/TOOL_PERMISSIONS.md)); a skill's `required_tools` must be a subset of the executing agent's grants. Skills can declare MCP-provided tools (MCP integration deferred — [../../Docs_COMPLEX/integrations/MCP.md](../../Docs_COMPLEX/integrations/MCP.md)). Bundled scripts run only in the sandboxed executor (ai-vfs), never host shell.
5. **Planner-reusable (deferred):** a plan step can be "apply skill X" — skills are the concrete form of the plan-pattern reuse noted in [../../Docs_COMPLEX/ai/PLANNING_ENGINE.md](../../Docs_COMPLEX/ai/PLANNING_ENGINE.md).
...
**Status:** Accepted (2026-06-13) — amends ADR-004, ADR-017 for the lean build
**Context:** For a solo, nights/weekends build, the role registry (ADR-004), three-profile system, and bootstrap capability gate (ADR-017) are ceremony ahead of need. Three free tiers are now available: Groq, Google AI Studio, and NVIDIA NIM (build.nvidia.com — DeepSeek-R1, Qwen, and other powerful models).
**Decision:** All LLM/embedding calls go through one LiteLLM seam; the model and an ordered fallback chain come from env/config (e.g. `groq → google → nvidia_nim`). No role registry, no profiles, no capability-gate bootstrap yet. NVIDIA NIM is OpenAI-compatible and treated as a rate-limited free tier in the chain. The full registry + capability gate (ADR-004/017) return when a real multi-profile swap needs them.
**Consequences:** (+) three independent free quotas = genuine rate-limit resilience at zero cost; minimal code. (−) provider neutrality is convention (config), not enforced at startup yet — re-add the gate when swaps get real.

## ADR-021 — pgvector instead of Qdrant; Redis/arq deferred (amends ADR-005, ADR-012)

**Status:** Accepted (2026-06-13) — amends ADR-005, ADR-012 for the lean build
**Context:** ADR-005 chose Qdrant + Postgres + Redis; ADR-012 added arq on Redis. At single-user scale, pgvector inside Postgres is sufficient and removes containers, and no background job queue exists until M6/consolidation.
**Decision:** One store — Postgres + **pgvector** — for operational data and semantic search through the lean build. Redis + arq deferred until a real job queue is needed. Qdrant remains the documented upgrade if pgvector measurably falls short.
**Consequences:** (+) one container; simpler ops. (−) pgvector ANN is less tuned than Qdrant at scale — fine for personal corpora; revisit if recall/latency degrade.

## ADR-022 — Defer full hexagonal ports to a single LLM seam (amends ADR-002)

**Status:** Accepted (2026-06-13) — amends ADR-002 for the lean build
**Context:** ADR-002's 18-port hexagon is team-sized discipline; for the lean build it is interface ceremony paid on every change. The one seam that has actually earned an abstraction is the LLM call (providers genuinely swap).
**Decision:** Keep one real seam — the LiteLLM call behind a single function — plus the natural Mem0 and ai-vfs boundaries. Add further ports reactively, the first time an implementation is actually swapped. import-linter CI deferred until there are layers to guard.
**Consequences:** (+) less ceremony, faster change. (−) less swap-ready by construction; the discipline returns port-by-port as needs prove out.

## ADR-023 — Drop Vision; defer Browser; re-sequence to BUILD_ORDER milestones (notes ADR-006)

**Status:** Accepted (2026-06-13)
**Context:** Vision (ADR-006) and the Browser agent are heavy subsystems not needed for a daily-use text assistant. Scope is re-sequenced around reaching daily use fast.
**Decision:** Vision is dropped from current scope (specs archived in `Docs_COMPLEX/`). Browser is deferred until a concrete recurring task justifies it. The build sequence is M0–M4 + M6 ([BUILD_ORDER.md](../BUILD_ORDER.md)); ROADMAP is rewritten to match. ADR-006 stands as the target design when Vision returns.
**Consequences:** (+) timeline aimed at daily use in ~6–8 weekends. (−) no image/screenshot or web-automation capability until those return.

## ADR-024 — Skills kept as the orchestration model, phased to the Supervisor (refines ADR-018, ADR-019)

**Status:** Accepted (2026-06-13) — refines ADR-018/019 (phasing only)
**Context:** A lean-build draft wrongly listed skills as deferrable ceremony. The progressive-disclosure skill pattern (ADR-018) is thin and is the chosen orchestration model — the Supervisor selects `{agent, skills}`.
**Decision:** Skills stay exactly as designed. They go live at **M3** with the Supervisor (registry + progressive disclosure + Level-1 selection + a few seed skills); M0–M2 agents are built skill-ready (the prompt contract accepts an injected SKILLS layer). Deferred: Level-2 mid-turn pulls, Planner composition (Phase 7), self-authoring, and the skill-selection eval. No change to the ADR-018/019 design — phasing only.
**Consequences:** (+) the orchestration model stays intact and is introduced exactly when the orchestrator exists. (−) no skill machinery before M3, which is correct — there is nothing to select against earlier.
