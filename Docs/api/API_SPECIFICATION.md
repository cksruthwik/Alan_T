...
Version: 0.2
Status: Active — lean scope ([BUILD_ORDER.md](../BUILD_ORDER.md)). FastAPI generates the executable OpenAPI; this doc fixes the contract shape and conventions.
...
**(server→client):** `{type:"token", text}` · `{type:"status", note}` ("searching your notes…") · `{type:"citation", vpath, position}` · `{type:"approval_request", request_id, tool, preview, risk}` ([ORCHESTRATION.md](../architecture/ORCHESTRATION.md) §1.5) · `{type:"done", turn_id, degraded:[]}` · `{type:"error", problem}`
...
| `/ingest/status` | GET | queue depth, done/pending counts, ETA ([KNOWLEDGE.md](../knowledge/KNOWLEDGE.md) §1) |
...
| `/health/deps` | GET | per-dependency status: postgres, groq, google, nvidia — feeds degradation flags |
| `/system/quota` | GET | current window budgets per provider/model ([LLM_STRATEGY.md](../ai/LLM_STRATEGY.md) §6) |
| `/system/audit` | GET | tool audit log (paginated, filterable) |
| `/system/models` | GET | active model-purpose → model config (read-only view) |

## 7. Deferred endpoints (arrive with their agents)

`/tasks*` and `/automations` (Task Planning + Automation agents). Shapes preserved in `Docs_COMPLEX/api/`.
