...
Version: 0.2
Status: Active — go live at **M3** with the Supervisor; M0–M2 agents built skill-ready ([BUILD_ORDER.md](../BUILD_ORDER.md))
Governing decisions: ADR-018, refined by ADR-024 (phasing) in [DECISION_LOG.md](../architecture/DECISION_LOG.md)
...
- **`model_hint`** lets a skill request a stronger model purpose (e.g. `REASONER`) for its turn, resolved through the LiteLLM seam ([LLM_STRATEGY.md](LLM_STRATEGY.md) §3).
...
Skills are **not owned by the orchestrator**. They live in a **Shared Resource Layer** alongside prompt templates, resources, and the tool catalog (ADR-019): **loaded once at startup, immutable at runtime, concurrently readable — no locks, no proxy hop through the Supervisor — by the Supervisor, every agent, and any model-using tool** ([AGENTS.md](../architecture/AGENTS.md) §5). Decentralizing access is the bottleneck fix: a multi-agent system must never serialize resource reads through one actor, and an agent must be able to reach a skill directly when a subtask needs it.
...
- Bundled `scripts/` run **only in the sandboxed executor** (ai-vfs, when available — [AI_VFS.md](../knowledge/AI_VFS.md)): VFS callbacks only, no host shell, budget-capped. Never the DENY'd `shell_exec`.
- Skills may later declare **MCP-provided tools** (MCP integration deferred); those would enter the registry and be gated like any tool.

## 6. Reuse by the Planner (deferred)

When the Task Planning Agent is built (deferred — [BUILD_ORDER.md](../BUILD_ORDER.md)), a plan step can be **"apply skill X"**: the Planner sees the same description menu and composes skills into a task DAG, and a successful ad-hoc plan can later be distilled into a skill (human-reviewed — §8). This is how the system's know-how compounds without bloating any single agent — noted now because the skill contract is designed to support it, but not built until the Planner exists.
...
- **M3 (skills go live):** registry + progressive disclosure + Supervisor Level-1 selection + prompt injection; a handful of seed skills (e.g. a note-summarization skill).
- **M4+:** productivity skills composing Notes/Calendar tools.
- **Deferred (with their agents):** Level-2 agent mid-turn pulls; Planner composition of skills; code-workflow skills.
- **Backlog:** self-authoring, user-defined skills with a sandbox/trust model.

M0–M2 agents are built **skill-ready** (the `Agent` contract accepts the injected SKILLS layer — [AGENTS.md](../architecture/AGENTS.md) §2), so turning skills on at M3 is additive, not a refactor.
